...
class UpdatePersonRequest extends FormRequest
{
    public function rules()
    {
        ...
        $type = $this->input('person.type') ?? [];

        $rules = [
            'person.id' => ['required', 'integer'],
            'person.name' => ['required', 'string', 'max:100'],
            'person.type' => ['required', 'string'],
            'pessoa.birth_date' => ['nullable', 'date'],
            'person.addresses' => ['sometimes', 'required', 'array'],
            'person.user' => ['required', 'array'],
            'person.user.*.name' => ['required'],
            'person.user.*.id' => ['required', 'integer'],
            'person.user.*.email' => ['required', 'email:rfc,filter,dns', 'max:100'],
            'person.addresses.*.id' => ['required', 'integer'],
            ...
            'person.addresses.*.is_default' => ['boolean'],
            ...
            'person.contacts.*.id' => ['required', 'integer'],

        ];

        if ($type == "student") {
            $rules['person.student'] = ['required', 'array'];
            $rules['person.student.*.id'] = ['required', 'email:rfc,filter,dns', 'max:100'];
            $rules['person.student.*.person.email_educacional'] = ['required', 'email:rfc,filter,dns', 'max:100'];

        }

        if ($type == "client") {
            $rules['person.client'] = ['required', 'array'];
            $rules['person.client.*.id'] = ['required', 'integer'];
            $rules['person.client.*.config'] = ['required', 'array'];
